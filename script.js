...
document.addEventListener("DOMContentLoaded", function gerarTabuleiro() {
  ...
  celula.forEach((cell) => {
    ...
    cell.addEventListener("mouseover", function shiningCell() {
      document
        .querySelectorAll(
          `.cell[data-column="${column}"], .cell[data-row="${row}"]`
        )
        .forEach((c) => {
    ...
    cell.addEventListener("click", function selectCell(event) {
      ...
      function addNum(event) {
        ...
        if (numClicado == numTab[dataRow - 1][dataColumn - 1]) {
          console.log(
            `O número ${numClicado} é igual ao número ${
              numTab[dataRow - 1][dataColumn - 1]
            }, ficar azul`
          );
          ...
          console.log(
            `O número ${numClicado} é DIFERENTE do nu ${
              numTab[dataRow - 1][dataColumn - 1]
            }, ficar Vermelho`
          );
      ...
      celula.forEach((c) => {
        ...
        if (
          (c.textContent == cell.textContent && cell.textContent !== "") ||
          idC == idCell
        ) {
